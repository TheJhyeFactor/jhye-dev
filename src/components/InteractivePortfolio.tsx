...
function OpenSourceView() {
  return <section className="portfolio-panel" aria-labelledby="opensource-title"><div className="panel-heading"><div><p className="panel-kicker">Fixes, findings, and why they matter</p><h2 id="opensource-title">Open source contributions</h2></div><p>{openSourceProjects.length} public notes</p></div><div className="open-source-list">{openSourceProjects.map((project, index) => <article key={project.title}><span>{String(index + 1).padStart(2, '0')}</span><div><p className="panel-kicker">{project.type}</p><h3>{project.title}</h3><p>{project.description}</p><dl><div><dt>What I fixed</dt><dd>{project.fixed}</dd></div><div><dt>Why</dt><dd>{project.why}</dd></div><div><dt>What improved</dt><dd>{project.improved}</dd></div></dl><div><a href={project.repo} target="_blank" rel="noreferrer">View repository <ArrowUpRight aria-hidden="true" /></a><a href={project.href} target="_blank" rel="noreferrer">Try the result <ArrowUpRight aria-hidden="true" /></a></div></div></article>)}</div></section>
}
