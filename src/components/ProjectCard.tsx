...
interface ProjectCardProps { title: string; description: string; image: string; tags: string[]; href?: string; year?: number; featured?: boolean }

export default function ProjectCard({ title, description, image, tags, href, year, featured }: ProjectCardProps) {
  const content = <>
    ...
    <div className="border-t border-[var(--border)] p-5 md:p-6">
      <div className="flex items-start justify-between gap-5"><div><p className="eyebrow mb-2">{featured ? 'Selected work' : 'Project'}</p><h3 className="font-['Space_Grotesk'] text-xl font-bold tracking-tight">{title}</h3></div>{href && <ArrowUpRight className="mt-1 h-5 w-5 shrink-0 text-[var(--accent)]" aria-hidden="true" />}</div>
